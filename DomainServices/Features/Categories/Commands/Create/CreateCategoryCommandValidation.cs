...
{
    public CreateCategoryCommandValidation(IReadOnlyRepository<Category> categoriesReadOnlyRepository)
    {
        ...
        RuleFor(x => x.Category.SectionsBelongsTo)
            ...
            .ForEach(coll =>
                coll.MustAsync(async (sectionCategoryBelongsTo, cancellationToken) =>
                    await categoriesReadOnlyRepository.ExistsAsync(existingSection =>
                        existingSection.Id == sectionCategoryBelongsTo.Id, cancellationToken)))
